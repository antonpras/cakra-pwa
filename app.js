document.addEventListener('DOMContentLoaded', () => {
    // Area untuk menampilkan status diagnosa
    const debugStatus = document.getElementById('debug-status');
    
    // Fungsi untuk memperbarui pesan status di layar
    function updateStatus(message, isError = false) {
        // Cek apakah elemen debugStatus ada di HTML
        const statusElement = document.getElementById('debug-status');
        if (statusElement) {
            statusElement.textContent = message;
            statusElement.style.color = isError ? '#dc3545' : '#28a745'; // Merah untuk error, hijau untuk sukses
        }
        // Juga tampilkan di console browser untuk debugging lebih lanjut
        if (isError) {
    ...
    try {
        ...
        updateStatus('Config Firebase dimuat.');

        // --- INITIALIZE FIREBASE ---
        firebase.initializeApp(firebaseConfig);
        updateStatus('Firebase diinisialisasi.');
        const auth = firebase.auth();
        updateStatus('Firebase Auth siap.');
        const db = firebase.firestore();
        updateStatus('Firestore siap.');
        const googleProvider = new firebase.auth.GoogleAuthProvider();
        updateStatus('Penyedia Google siap.');
        ...
        updateStatus('Semua elemen DOM dipilih.');
        ...
        auth.onAuthStateChanged(user => {
            updateStatus(user ? `User terdeteksi: ${user.email}` : 'Tidak ada user yang login.');
            if (user) {
        ...
        loginBtn.addEventListener('click', () => {
            updateStatus('Tombol Login Ditekan! Mencoba redirect...');
            try {
                auth.signInWithRedirect(googleProvider);
            } catch (error) {
                updateStatus(`ERROR saat signInWithRedirect: ${error.message}`, true);
            }
        });
        ...
        function renderApp() {
            ...
            const sortedEntries = entriesToRender.sort((a, b) => (b.isPinned - a.isPinned) || (b.updatedAt.toMillis() - a.updatedAt.toMillis()));
        ...
        updateStatus(`ERROR KRITIS: ${error.message}`, true);
    }
});
